# MySQL Binlog Lock Splitting Feature Guide

## Feature Description<a name="EN-US_TOPIC_0000002518537838"></a>

### Overview<a name="EN-US_TOPIC_0000002518537840"></a>

This document describes how to install and enable the binlog lock splitting feature on a Kunpeng server.
...
|Percona|<ul><li>Percona-Server 5.7.44-53</li><li>Percona-Server 8.0.43-34</li></ul>|<ul><li>[Link](https://gitcode.com/boostkit/boostdb/releases/download/MySQL-Percona-Server-5.7.44-53-v3/BoostDB-Percona-5.7.44-53.aarch64.rpm)</li><li>[Link](https://gitcode.com/boostkit/boostdb/releases/download/MySQL-Percona-Server-8.0.43-34-v2/BoostDB-Percona-8.0.43-34.aarch64.rpm)</li></ul>|
...
5. (Optional) Perform the sysbench test to compare the performance before and after the binlog lock splitting feature is enabled. For details about the test procedure, see [Sysbench 0.5 & 1.0 Test Guide](https://www.hikunpeng.com/document/detail/en/kunpengdbs/testguide/tstg/kunpengsysbench_02_0001.html).<br>Binlog optimization (three features combined) improves the sysbench write performance by 13%. [**Figure 1**](#performance-comparison) shows the performance before and after the optimization.
    ...
    **Figure 1** Performance comparison before and after binlog optimization (three features combined)<a name="fig937192253919"></a><a id="performance-comparison"></a><br>
    ![](figures/performance_comparison_binlog.png "Performance comparison before and after binlog lock splitting")
