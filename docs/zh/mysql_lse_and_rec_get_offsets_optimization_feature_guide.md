# MySQL LSE及rec_get_offsets优化 特性指南

## 特性描述<a name="ZH-CN_TOPIC_0000002511086264"></a>

### 简介<a name="ZH-CN_TOPIC_0000002542566231"></a>

本文主要介绍如何在鲲鹏服务器上安装和使能MySQL LSE（Large System Extensions）及rec\_get\_offsets优化特性。

在MySQL OLTP场景中，高并发读写下，出现锁竞争激烈导致性能呈现线性劣化的情况。LSE是专为现代多核、高并发环境设计的原子操作硬件加速方案。它通过单指令原子操作，解决了传统LL/SC（Load-Link/Store-Conditional）指令在扩展性上的瓶颈。另外，rec\_get\_offsets函数每次被调用时都需要通过dict\_table\_is\_comp来查询表的行格式是否为compact，可提前获取行格式判断结果，并将其作为参数传递给重构后的函数，以提升记录的读取效率。

### 原理描述<a name="ZH-CN_TOPIC_0000002511086262"></a>

本特性通过引入LSE硬件加速和优化rec\_get\_offsets的调用逻辑，来有效缓解高并发下的性能劣化问题，提升MySQL在鲲鹏服务器中的整体性能表现与系统稳定性。
...
**图 1** LL/SC指令（ldaxr&stlxr）<a name="fig1834732512417"></a><a id="LL/SC指令（ldaxr&stlxr）"></a><br>
![](figures/ll_sc指令（ldaxr_stlxr）.png "LL-SC指令（ldaxr-stlxr）")

**图 2** LSE指令（ldaddal）<a name="fig12283173611418"></a><a id="LSE指令（ldaddal）"></a><br>
![](figures/LSE指令（ldaddal）.png "LSE指令（ldaddal）")

**rec\_get\_offsets优化<a name="section695124719482"></a>**

在InnoDB存储引擎中，rec\_get\_offsets函数用于确定物理记录（rec\_t）中每个字段的字节偏移量。在原实现中，该函数每次被调用时都需要通过dict\_table\_is\_comp来查询表的行格式是否为compact，由于该函数在物理记录读取过程中调用频繁，这一重复判断带来了额外的开销。为了提升性能，本特性在调用rec\_get\_offsets前获取行格式判断结果，并将其作为参数传递给新增的rec\_get\_offsets\_with\_comp函数。新函数直接使用传入的compact标志，避免了内部重复查询，从而减少了调用开销，提高了InnoDB存储引擎在处理记录时的效率。
...
以Percona-Server 5.7.44-53为例介绍如何安装和使能MySQL LSE及rec\_get\_offsets优化特性，具体操作步骤如下。
...
5. （可选）通过Sysbench测试可以得到使能本特性前后的性能提升效果，详细测试步骤请参见《[Sysbench 0.5&1.0 测试指导](https://www.hikunpeng.com/document/detail/zh/kunpengdbs/testguide/tstg/kunpengsysbench_02_0001.html)》。MySQL LSE及rec\_get\_offsets优化特性可以使Sysbench 8U16G规格下256并发综合性能（只读、读写、只写）提升5%，优化前后对比效果如[**图 3** MySQL LSE及rec\_get\_offsets优化特性优化前后性能对比](#mysql-lse-rec-get-offsets-perf-compare)所示。

   **图 3** MySQL LSE及rec\_get\_offsets优化特性优化前后性能对比<a name="fig937192253919"></a><a id="mysql-lse-rec-get-offsets-perf-compare"></a><br>
   ![](figures/mysql_lse_rec_get_offsets_perf_compare.png "MySQL-LSE及rec_get_offsets优化特性优化前后性能对比")
