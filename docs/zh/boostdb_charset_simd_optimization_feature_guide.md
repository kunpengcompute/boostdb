...
在代码实现层面，不同Percona版本的SIMD优化范围对应如下。

- Percona-Server-5.7.44-53：
  - 在utf8/utf8mb4的general_ci路径上，优化`my_strnxfrm_unicode`、`my_hash_sort_utf8`、`my_hash_sort_utf8mb4`，并在字符集处理路径优化`my_numchars_mb`、`my_charpos_mb`。
  - 在`utf8_unicode_ci`和`utf8mb4_unicode_ci`（UCA）路径上，也启用了SVE优化，主要覆盖 UCA 比较、哈希和权重转换相关流程（`my_uca_simd_support_check`、`my_hash_sort_uca`、`my_strnxfrm_uca`）。
...
3. 启动数据库。启动数据库的操作请参见《MySQL移植指南》的[运行MySQL](https://www.hikunpeng.com/document/detail/zh/kunpengdbs/ecosystemEnable/MySQL/kunpengmysql8017_03_0013.html)章节。
...
5. （可选）通过Sysbench测试可以得到使能字符集处理SIMD优化特性前后的性能提升效果，详细测试步骤请参见《[Sysbench 0.5&1.0 测试指导](https://www.hikunpeng.com/document/detail/zh/kunpengdbs/testguide/tstg/kunpengsysbench_02_0001.html)》。<br>通过记录匹配优化、字符集处理的SIMD和非对齐内存访问优化三特性叠加，可以使Sysbench只读场景性能提升10%，优化前后对比效果如[图 三特性叠加优化前后性能对比](#计算路径优化特性优化前后性能对比)所示。
